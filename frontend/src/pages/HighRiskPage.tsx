...
export default function HighRiskPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<QueryAnalysis[]>("/query/high-risk")
      .then((res) => setItems(res.data))
      .catch((err) => {
        if (err?.response?.status === 401) {
          navigate("/login");
        } else {
          setError("Unable to load high-risk queries.");
        }
      })
      ...
      <div className="space-y-4">
        <header className="space-y-1">
          ...
          <p className="text-xs text-slate-400 max-w-xl">
            Queries whose risk score exceeds your configured high-risk
            threshold. Review these carefully.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <p className="text-xs text-slate-500">
            You don&apos;t have any high-risk queries yet.
          </p>
        ) : (
